 ...
 const params = e.detail;
  ...
  class Level extends Phaser.State {
    preload() {
      ...
      this.game.load.spritesheet('sprites', '/images/cbimage.png', SCREEN_BLOCK, SCREEN_BLOCK);
    }
      ...
      MAP.forEach(function(row, x){
        row.forEach(function(m, y){
          if (m == "1") {
            ...
            switch(m){
              case "2":
                const bricks = new Bricks(self.game, x * self.grid.size, y * self.grid.size, self.grid);
                self.items.add(bricks);
                break;
              default:
                null
            }
  ...
  window.UPDATE_LEVEL = function(self){
    self.game.physics.arcade.collide(self.player, self.items, (a, b) => {
  ...
  window.addEventListener(`phx:join_players`, function({detail: payload}){
    setTimeout(function(){
      PLAYERS = payload.players;
      const level = game.state.states.Level;
      const me = payload.players.filter(p => p.associated == ME);
      if(me.length > 0 && !IS_PLAYER){
        const player = new Player(me[0], level, level.grid);
        level.items.add(player);
        window.UPDATE_LEVEL = function(self){
          game.physics.arcade.collide(player, level.items, (a, b) => {
            if (a instanceof Player && (b instanceof Blast || b instanceof Explosion)) {
              a.kill();
            }
          });
        ...
        const others = payload.players.filter(p => p.associated != ME && !(p.associated in PLAYERS_ADDED));
        others.forEach(function(p){
          const other = new OtherPlayer(p, level, level.grid);
          level.items.add(other);
          PLAYERS_ADDED.push(p.associated)
          CURRENT_OTHER_PLAYERS.push(other)
        })
      }
    }, 500)
  });
  window.addEventListener(`phx:leave_players`, function({detail: payload}){
    setTimeout(function(){
    }, 500)
  });
